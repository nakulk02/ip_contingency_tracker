...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function fetchGaps() {
      const res = await fetch("/api/dashboard/ownership-gaps");
      if (res.ok) {
        ...
        <Card className="border-0 shadow-md bg-gradient-to-br from-emerald-50 to-emerald-100/50">
          ...
          <CardContent>
            <div className="text-3xl font-bold text-emerald-700">
              {gaps.assetsAtRisk === 0 && gaps.peopleWithoutAgreements === 0
                ? "100%"
                : "Gaps found"}
            </div>
          ...
          <CardContent>
            {gaps.assets.length === 0 ? (
              <div className="text-center py-8 text-muted-foreground">
                ...
                <p className="text-sm font-medium">All assets covered</p>
              </div>
            ) : (
              <Table>
                <TableHeader>
                ...
                <TableBody>
                  {gaps.assets.slice(0, 5).map((asset) => (
                    <TableRow key={asset.id}>
                      <TableCell>
                        <Link href={`/ip-assets/${asset.id}`} className="text-primary hover:underline font-medium text-sm">
                          {asset.title}
                        </Link>
                      </TableCell>
                      <TableCell className="text-sm">{asset.type}</TableCell>
                      <TableCell>
                        <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${statusColors[asset.status] || ""}`}>
                          {asset.status}
                        </span>
                      </TableCell>
                    </TableRow>
          ...
          <CardContent>
            {gaps.people.length === 0 ? (
              <div className="text-center py-8 text-muted-foreground">
                ...
                <p className="text-sm font-medium">Everyone has agreements</p>
              </div>
            ) : (
              <Table>
                <TableHeader>
                ...
                <TableBody>
                  {gaps.people.slice(0, 5).map((person) => (
                    <TableRow key={person.id}>
                      <TableCell>
                        <Link href={`/people/${person.id}`} className="text-primary hover:underline font-medium text-sm">
                          {person.name}
                        </Link>
                      </TableCell>
                      <TableCell>
                        <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${roleColors[person.role] || ""}`}>
                          {person.role}
                        </span>
                      </TableCell>
                      <TableCell>
